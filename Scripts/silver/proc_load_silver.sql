    ...
    BEGIN TRY
		...
		SELECT
			...
			CASE 
				...
				ELSE 'n/a'
			END AS cst_gndr, -- Normalize cust_gender values to readable format
			cst_create_date
		FROM (
			SELECT
				*,
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS category_id, -- Extract categoryegory ID
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS cust_gen -- Normalize cust_gender values and handle unknown cases
